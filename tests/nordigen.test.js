...
const NordigenClient = require('../nordigen');
...
describe('Test Client', () => {
  ...
  it('Test agreement', async () => {
    ...
    expect(agreementDelete.detail).toBe(
      `End User Agreement ${agreementId} deleted`
    );
  });
  ...
  it('Test requisition api', async () => {
    ...
    // Delete requisition
    const deletedRequisition = await client.requisition.deleteRequisition(
      requisitionId
    );
    expect(deletedRequisition.detail).toBe(
      `Requisition ${requisitionId} deleted with all its End User Agreements`
    );
  });
